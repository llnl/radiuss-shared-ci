...
.. list-table::
   ...
   * - ``github_project_name``
     - Yes
     - GitHub project name (e.g., "RAJA")
   * - ``github_project_org``
     - Yes
     - GitHub organization (e.g., "LLNL")
   * - ``github_token``
     - No
     - GitHub API token for status updates

machine-pipeline (all machines)
================================
...
.. list-table::
   ...
   * - ``job_cmd``
     - Yes
     - Command to build and test project
   * - ``github_project_name``
     - Yes
     - GitHub project name
   * - ``github_project_org``
...
.. list-table::
   ...
   * - ``shared_alloc``
     - Yes
     - Shared allocation args or "OFF"
   * - ``job_alloc``
     - Yes
     - Per-job allocation args
   * - ``alloc_name``
     - No
     - Name for shared allocation (default: "ci_<machine>")
...
.. list-table::
   ...
   * - ``perf_processing_cmd``
     - Yes
     - Results processing command
   * - ``<machine>_perf_alloc``
     - No
     - Per-machine allocation (e.g., ``dane_perf_alloc``)
   * - ``perf_artifact_dir``
     - No
     - Artifacts directory (default: ".")
   * - ``perf_results_file``
     - No
...
.. list-table::
   ...
   * - ``always_run_pattern``
     - No
     - Regex for branches that always run (e.g., ``"^(main|develop)$"``)
...
- ``.machine-check`` - Machine availability check job
- ``.build-and-test`` - Child pipeline trigger template
- ``.custom_job`` - Base job customization template
- ``.custom_perf`` - Performance job customization template

From machine-pipeline
=====================
...
- ``.custom_perf`` - Customization template for perf jobs
